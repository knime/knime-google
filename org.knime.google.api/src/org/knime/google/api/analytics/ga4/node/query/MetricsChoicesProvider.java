...
/**
 * Synchronous choices provider for fetching available metrics from Google Analytics 4.
 *
 * @author Manuel Hotz, KNIME GmbH, Konstanz, Germany
 */
@SuppressWarnings("restriction") // webui* classes
final class MetricsChoicesProvider implements StringChoicesProvider {
    ...
    public List<String> choices(final NodeParametersInput ctx) {
        final var connSpec = GAQueryNodeModel.getGoogleAnalyticsConnectionPortObjectSpec(ctx.getInPortSpecs());
        if (connSpec.isPresent()) {
            try {
                final var pos = connSpec.get();
                final var conn = pos.getConnection();
                return conn.metadata(pos.getProperty()).getMetrics().stream().map(MetricMetadata::getApiName).toList();
            } catch (KNIMEException e) {
                LOGGER.error("Failed to retrieve Google Analytics 4 metrics from Google Analytics API.", e);
            }
        }
