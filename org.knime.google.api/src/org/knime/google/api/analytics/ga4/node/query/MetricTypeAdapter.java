...
/**
 * Adapter to parse Google Analytics 4 Data API v1
...
final class MetricTypeAdapter {
    ...
    static Optional<MetricTypeAdapter> lookup(final String type) {
        final Set<String> doubleTypes = Set.of(
        ...
        final var doubleAdapter = new MetricTypeAdapter(DoubleCell.TYPE, s -> new DoubleCell(Double.parseDouble(s)));
        /* Unspecified type. */
        if ("METRIC_TYPE_UNSPECIFIED".equals(type)) {
            return Optional.of(new MetricTypeAdapter(StringCell.TYPE, StringCell::new));
        } else if ("TYPE_INTEGER".equals(type)) {
            return Optional.of(new MetricTypeAdapter(IntCell.TYPE, s -> new IntCell(Integer.parseInt(s))));
        } else if (doubleTypes.contains(type)) {
            return Optional.of(doubleAdapter);
        }
