...
public class GoogleCloudStorageClient {
    ...
    public List<Bucket> listBuckets() throws IOException {
        return m_storage.buckets().list(m_projectId).execute().getItems();
    }

    /**
     * Returns list of object for a given bucket.
     *
     * @param bucket
     *            bucket name.
     ...
     * @throws IOException
     */
    public List<StorageObject> listObjects(final String bucket) throws IOException {
        return m_storage.objects().list(bucket).execute().getItems();
    }
